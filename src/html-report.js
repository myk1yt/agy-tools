...
function renderDashboardHtml(payload, opts = {}) {
  ...
  const clientScript = [
    ...
    "      return;",
    "    }",
    "    // Design 000815 §4.3/§4.4: real quota snapshot outranks the rolling ESTIMATE",
    "    // gauge even when stale/auth-failed; keep it, never demote to (r) branch.",
    "    var canUseLiveGq = Boolean(gq && gq.remainPercent !== null && gq.remainPercent !== undefined);",
    ...
    "      html += liveBadge;",
    "    } else if (r) {",
    "      html += '<span style=\"font-size:10px;padding:2px 6px;background:rgba(139,148,158,0.15);color:#8b949e;border:1px solid var(--border);border-radius:10px;\">' + esc(I18N.quotaFallback || 'ESTIMATE') + '</span>';",
    "    }",
    "    html += '</div>';",
    "    if (canUseLiveGq && (gq.quota5h || gq.quota7d)) {",
    ...
    "          '</div>';",
    "      }",
    "    } else if (canUseLiveGq && gq && gq.remainPercent !== null && gq.remainPercent !== undefined) {",
    ...
    "    renderEstimates(p);",
    "    renderQuota(p ? p.rollingUsage : null, p ? p.geminiQuota : null);",
    "  }",
    "",
    "  function setLive(on) {",
    ...
    "      if (typeof done === 'function') done();",
    "    }",
    "    var isHttp = typeof window !== 'undefined' && window.location && window.location.protocol && window.location.protocol.indexOf('http') === 0;",
    "    var dataUrl = isHttp",
    "      ? '/data.json?v=' + Date.now()",
    "      : ('http://127.0.0.1:' + activePort + '/data.json?v=' + Date.now());",
    ...
    "              } catch (_e) {}",
    "            }",
    "            fallbackScriptPoll(finish);",
    "          }",
    "        };",
    "        xhr.onerror = function () { fallbackScriptPoll(finish); };",
    "        xhr.send();",
    "        return;",
    "      } catch (_err) {",
    "        fallbackScriptPoll(finish);",
    ...
    "    fallbackScriptPoll(finish);",
    "  }",
    "  function fallbackScriptPoll(finish) {",
    "    var isHttp = typeof window !== 'undefined' && window.location && window.location.protocol && window.location.protocol.indexOf('http') === 0;",
    "    var sc = document.createElement('script');",
    "    sc.src = isHttp ? ('dashboard-data.js?v=' + Date.now()) : 'dashboard-data.js';",
    "    sc.onload = function () {",
    "      if (isFreshPayload(window.__AGY_DASH__)) {",
    "        render(window.__AGY_DASH__);",
    "        setLive(true);",
    "        lastPayload = window.__AGY_DASH__;",
    "      } else if (lastPayload) {",
    ...
    "  }",
    "  function startPolling() { if (!pollTimer) pollTimer = setInterval(function() { pollOnce(); }, REFRESH_MS); }",
    ...
    "    }",
    "  }",
    "  function stopBgSync() { if (bgSyncTimer) { clearInterval(bgSyncTimer); bgSyncTimer = null; } }",
    "",
    "  var currentEs = null;",
    "  var sseActive = false;",
    "  var sseAttempt = 0;",
    ...
    "      return '/events';",
    "    }",
    "    if (sseActive || sseAttempt === 0) {",
    "      return sseUrlForPort(activePort);",
    "    }",
    "    var portOffset = (sseAttempt - 1) % 5;",
    "    return sseUrlForPort(SSE_PORT_HINT + portOffset);",
    "  }",
    "  function cleanupSse() {",
    "    if (reconnectTimer) {",
    ...
    "    startPolling();",
    "    sseAttempt++;",
    "    var delay = Math.min(15000, Math.round(1000 * Math.pow(1.5, Math.min(sseAttempt, 8))));",
    ...
    "        try {",
    "          if (es.url) {",
    "            var m = es.url.match(/:(\d+)\//);",
    "            if (m && m[1]) activePort = Number(m[1]);",
    "          }",
    "        } catch (_e) {}",
    "        stopPolling();",
    "        startBgSync();",
