...
function renderDashboardHtml(payload, opts = {}) {
  ...
  const clientScript = [
    ...
    "      return '/events';",
    "    }",
    "    return sseAttempt === 0 ? SSE_URL : sseUrlForPort(SSE_PORT_HINT + sseAttempt);",
    "  }",
    "  function trySse() {",
    "    try {",
    "      var url = getSseUrl();",
    ...
    "            reconnectWatchdog = setTimeout(function () {",
    ...
    "        if (sseAttempt < 10) {",
    "          sseAttempt += 1;",
    "          var delay = Math.min(10000, 1000 * Math.pow(1.5, sseAttempt - 1));",
    "          setTimeout(trySse, delay);",
    "        }",
    "      };",
